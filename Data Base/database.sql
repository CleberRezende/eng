...
CREATE TABLE tb_OPCIONAL_CARRO(
	FK_CAR_CODIGO	INT FOREIGN KEY REFERENCES tb_CARRO(CAR_CODIGO),
	OPC_QTD_PORTA	INT	CONSTRAINT chk_OPC_QTD_PORTA CHECK (OPC_QTD_PORTA IN (2,4)),
	OPC_AR_CONDIC	VARCHAR(50) CONSTRAINT chk_OPC_AR_CONDIC CHECK(OPC_AR_CONDIC IN('SIM','NÃO')),
	OPC_RODA_LIGA	VARCHAR(50) CONSTRAINT chk_OPC_RODA_LIGA CHECK(OPC_RODA_LIGA IN('SIM','NÃO')),
	OPC_VIDRO_ELET	VARCHAR(50) CONSTRAINT chk_OPC_VIDRO_ELET CHECK(OPC_VIDRO_ELET IN('SIM','NÃO')),
	OPC_TRAVA_ELET	VARCHAR(50) CONSTRAINT chk_OPC_TRAVA_ELET CHECK(OPC_TRAVA_ELET IN('SIM','NÃO')),
	OPC_CD_PLAYER	VARCHAR(50) CONSTRAINT chk_OPC_TRAVA_ELET CHECK(OPC_TRAVA_ELET IN('SIM','NÃO'))
)
